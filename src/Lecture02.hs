...
filter f x = foldr (\y z -> if f y then y :. z else z) Nil x
...
reverse x = foldl (\xs x -> x :. xs) Nil x 
...
toListH :: List a -> [a]
toListH Nil = []
toListH (x :. y) = x : toListH y

-- И обратно
fromListH :: [a] -> List a
fromListH [] = Nil
fromListH (x : y) = x :. fromListH y
